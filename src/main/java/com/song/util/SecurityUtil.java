...
public class SecurityUtil {
	...
	public static String encode(String xmlStr) {
		...
		byte[] md5Hasn = null;
		try {
			md5Hasn = SecurityUtil.MD5Hash(encrypt, 0, encrypt.length);
		} catch (Exception e) {
			e.printStackTrace();
		}
		// 组合消息体
		...
		byte[] key = new byte[8];
		byte[] iv = new byte[8];
		getKeyIV(SecurityUtil.key, key, iv);
		...
		byte[] temp = null;
		try {
			temp = SecurityUtil.DES_CBC_Encrypt(totalByte, deskey, ivParam);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @throws Exception
	 */
	public static String decode(String xmlStr) throws Exception {
		// base64解码
		...
		byte[] temp = null;
		try {
			temp = SecurityUtil.DES_CBC_Decrypt(encBuf, deskey, ivParam);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		byte[] md5Hash = null;
		try {
			md5Hash = SecurityUtil.MD5Hash(temp, 16, temp.length - 16);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		try{
			...
			System.out.println(decode("oqGiG3w2C/s4l945xI++My4Wpv2cCyLi"));
		}catch (Exception e){
			e.printStackTrace();
		}
	}
